...
const roiPerTenLakh = [
  { year: 1, label: "Establishment", flow: -1000000 },
  { year: 2, label: "Establishment", flow: -40000 },
  { year: 3, label: "First light yield", flow: 120000 },
  { year: 4, label: "Yield building", flow: 250000 },
  { year: 5, label: "Yield building", flow: 400000 },
  { year: 6, label: "Approaching peak", flow: 550000 },
  { year: 7, label: "Near peak", flow: 680000 },
  { year: 8, label: "Peak production", flow: 750000 },
  { year: 9, label: "Peak production", flow: 780000 },
  { year: 10, label: "Peak production", flow: 800000 },
];
  ...
  return (
    <div>
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="ledger-rule-b text-left text-forest/70">
              ...
              <th className="py-2 pr-4 font-normal text-right">Net cash flow</th>
              <th className="py-2 font-normal text-right">Cumulative</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r, i) => (
              <tr key={r.year} className="ledger-rule-b">
                <td className="py-2 pr-4 step-number">{r.year}</td>
                <td className="py-2 pr-4 text-ink/70">{r.label}</td>
                <td
                  className={`py-2 pr-4 text-right tabular-nums ${
                    r.flow < 0 ? "text-rust" : "text-forest"
                  }`}
                >
                  {formatINR(r.flow)}
                </td>
                <td className="py-2 text-right tabular-nums font-medium">
                  {formatINR(cumulative[i])}
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <p className="mt-6 font-display text-2xl">
        Illustrative 10-year position: {formatINR(finalValue)} on {formatINR(investment)} invested
      </p>
